...
# Essential packages
pkgs <- c("starc", "dplyr", "jsonlite", "reproj", "sds", "gdalraster", "vapour", "crew")
...
list(
  # STAC Configuration
  tar_target(provider, "https://earth-search.aws.element84.com/v1/search"),
  tar_target(collection, "sentinel-2-c1-l2a"),

  # test locations, includes antimeridian
  tar_target(locations, define_test_location() |> prepare_locations_clean()),
  tar_target(assets_parquet_store, "_targets/assets_parquet"),
  starc_targets_assets()

  # ## use the full pipeline, or just the factory above as starc_targets_assets()
  # # =============================================================================
  # # CONFIGURATION
  # # =============================================================================
  #
  ...
  # query_specs <- prepare_query(spatial_window[1, ], collection, provider) |>
